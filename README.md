...
- Прописать настройки в `setting.ini` :
	- Настройки общей папки с БД Meter.db
		- Путь до общей папки в графе `dbpath`
		- Папка в самом дитрибутиве находиться по пути : `/var/opt/uspd`
	- Настройки IP адресса коннкета
...
Данные тесты можно запускать с параметрами взависимости от того, где находится дистибутив с установленым нужным компонетом, тест которого проводится

**Поддержанные способы связи:**

- Запуск через локально запущеный docker контейнер 
- Запуск через гостевую виртуальную машину VirtualBox
- Запуск с помощью коннекта по TCP\IP
- Запуск внутри linux-системы

### Запуск через запущенный docker контейнер
- Для этого способа необходимо **локально** запустить docker контейнер в котором будет установлен нужный компонет.
- Для работы с этим способом необходима установка модуля API для Docker(Команда указана в разделе Зависимости)
- Скрипт поключиться через Docker exec в **первый** запущенный контейнер - и будет взаимодействоваать с ним
