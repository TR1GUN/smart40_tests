...
class VirtualMeter(JOB):
   ...
   Здесь расположим всю основную работу со счетчиком
    ...
    def __EmulatorMeter(self):
        ...
        for self.count_thread in range(self.thread):
    ...
    # ---------------------------------КАНАЛ ВЗАИМОДЕЙСТВИЯ - ETHERNET ------------------------------------------------
    # -----------------------------------------------------------------------------------------------------------------
    def iface_Ethernet(self, job_type: str, port: int = 7777, thread: int = 2):
        ...
        for i in range( self.thread):
            ...
            self.ports['thread_' + str(i)] = int(self.port + i)
        ...
        self.JSON_dict = {}
        for i in range(self.thread):
            self.JSON_dict['thread_' + str(i)] = GeneratorJob(job=job_type,
                                      iface='Ethernet',
                                      address_settings=str(ipconfig) + str(self.port + i),
...
test = VirtualMeter(type_connect='virtualbox').iface_Ethernet(job_type='ElDayEnergy' , thread=4)
# print(test)
