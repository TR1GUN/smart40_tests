# Итак . Здесь расположим парсер JSON ответа до вида равному селекта из БД


class ParseAnswerMeterDataJSON:
    JSON = {}
    JSON_deconstruct = []
    ...
    def __init__(self, JSON):
        ...
        if JSON["res"] == 0:
            ...
            #     self.JSON_deconstruct[i] = dict(sorted(self.JSON_deconstruct[i].keys() , reverse=True))
    ...
    def __parse_measures(self):
        ...
        for i in range(len(measures)):
            ...
            for x in range(len(measures_list)):
                measures_list[x]["measure"] = measures[i]["measure"]
            measures_list_full = measures_list_full + measures_list
    ...
    def __parse_devices(self):
        ...
        if devices is None:
            ...
            for i in range(len(devices)):
                ...
                # внутрений айдишник
                if "deviceIdx" in devices[i]:
                    devices_dict["deviceIdx"] = devices[i]["deviceIdx"]
